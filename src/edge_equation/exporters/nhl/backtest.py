"""NHL game-results backtest CLI.

Thin wrapper around `_game_results_backtest.GameResultsBacktestEngine`
with `NHL_CONFIG`. NHL goals/game (~6.2 league avg) and SD (~2 goals)
are tighter than NBA -- see _game_results_backtest.NHL_CONFIG.

Usage::

    python -m edge_equation.exporters.nhl.backtest --seasons 2024 2025
    python -m edge_equation.exporters.nhl.backtest --seasons 2024 2025 \\
        --output data/nhl_backtest_summary.json
"""
...
from edge_equation.exporters._game_results_backtest import (
    GameResultsBacktestEngine, NHL_CONFIG, load_games_jsonl,
)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(prog="edge_equation.exporters.nhl.backtest")
    ...
    print(f"Loaded {len(rows):,} NHL games from seasons {args.seasons}")
    if not rows:
        print("No data on disk. Run scripts/backfill_nhl_games.py first.")
        return 1
    eng = GameResultsBacktestEngine(rows=rows, cfg=NHL_CONFIG)
    res = eng.run()
